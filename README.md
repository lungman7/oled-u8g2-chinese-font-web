...
1. 使用者按下授權後，網頁透過 `queryLocalFonts()` 取得 Windows 全部已安裝
   字型的名稱、家族與樣式，並放入可搜尋的下拉選單。
2. 列出名稱時不讀取字型檔；只有使用者選定某個字型後，才呼叫該
   `FontData` 的 `blob()` 取得實際字型資料，交由 Canvas 繪製輸入文字。
3. 瀏覽器將點陣組成 BDF；不會上傳完整的本機字型檔。
4. Flask 後端驗證輸入，再呼叫 U8g2 官方 `bdfconv` 產生 C 字型陣列。
5. 下載檔案已含 `#pragma once`、來源資訊與中文使用說明。

完整本機字型列舉屬於瀏覽器的高權限功能，必須使用桌面版 Chrome／Edge、
由使用者明確授權，並從 HTTPS 安全網址開啟。HTTP 或不支援的瀏覽器可改用
